function TitleStatusListController(
    $controller,
    $q,
    $http,
    $rootScope,
    ...
    $scope.GetDataCore = function (model, tableState) {
        model.UserId = appSession.GetCurrent().UserId;
        return itemService.GetTitleStatuses(model);
    }
    ...
    $scope.ValidateRowCore = function (row) {
        var isValid = true;
        isValid = !$scope.manufacturerForm.$invalid;
        if (row != null) {
    ...
    $scope.SaveCore = function (changes) {
        ...
        itemService.SaveTitleStatuses(changes)
    ...
    $scope.Broadcast = function () {
        if (window.$windowScope != null) {
            window.$windowScope.$root.$broadcast("TitleStatusChange", null);
        }
    }
    $scope.DoInit();
}